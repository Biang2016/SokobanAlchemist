...
public sealed class AroundMapGenerator : MapGenerator
{
    ...
    public override void ApplyToWorldMap()
    {
        ...
        {
            TryOverrideToWorldMap(new GridPos3D(world_x, Height, 0));
            TryOverrideToWorldMap(new GridPos3D(world_x, Height, Depth - 1));
        }

        for (int world_z = 0; world_z < Depth; world_z++)
        {
            TryOverrideToWorldMap(new GridPos3D(0, Height, world_z));
            TryOverrideToWorldMap(new GridPos3D(Width - 1, Height, world_z));
        }
    }
}